...
def update_gui(name):
    attendance_label.config(text=f"{name} Present", bg="#b3cde0")
...
def update_camera_feed(known_face_encodings, known_face_names):
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        if matches[best_match_index] and face_distances[best_match_index] < 0.4:  # Adjust threshold here
...
# Title label
title_label = tk.Label(root, text="Face Recognition Attendance System", font=('Helvetica', 20, 'bold'), bg="#03396c", fg="white")
title_label.pack(pady=10)

# Date and time label
current_datetime = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
datetime_label = tk.Label(root, text=current_datetime, font=('Helvetica', 14), bg="#011f4b", fg="white")
datetime_label.pack()

# Create a label for attendance display
attendance_label = tk.Label(root, text="", font=('Helvetica', 18), bg="#011f4b", fg="white")
attendance_label.pack(pady=20)

# Create a label for camera feed
camera_label = tk.Label(root, bg="#011f4b")
camera_label.pack()

# Button to stop the camera and close the window
stop_button = tk.Button(root, text="Stop and Exit", command=stop_camera, font=('Helvetica', 14), bg="#005b96", fg="white", activebackground="#03396c", activeforeground="white")
